...
const GAME_CONFIG = {
    ...
    bulletSpeed: 7, bulletRadius: 4, bulletDamage: 30, attackCooldown: 40,
    enemySpeed: 1.5, enemyRadius: 10, enemyMaxHp: 100, enemySpawnRate: 40, maxEnemies: 100,
...
const player = { x: 0, y: 0, hp: 100, xp: 0, level: 1, nextLevelXp: 100, attackTimer: 0, color: '#00ff00' };
...
function init() {
    ...
    // Create Pools
    for(let i=0; i<GAME_CONFIG.maxEnemies; i++) enemies.push({ active: false, x:0, y:0 });
    for(let i=0; i<50; i++) bullets.push({ active: false, x:0, y:0 });
    for(let i=0; i<100; i++) gems.push({ active: false, x:0, y:0 });
...
function startGame() {
    ...
    GAME_CONFIG.bulletDamage = 30;
    GAME_CONFIG.playerSpeed = 3;
    GAME_CONFIG.magnetRadius = 100;
    GAME_CONFIG.playerMaxHp = 100;

    player.hp = 100; player.xp = 0; player.level = 1; player.nextLevelXp = 100;
    score = 0;
    frameCount = 0;
...
// === Game Logic ===
function update() {
    if (gameState !== 'PLAYING') return; // 일시 정지
    ...
    if (player.attackTimer > 0) player.attackTimer--;
        ...
        enemies.forEach(e => {
            ...
            if (d < canvas.width/2 && d < minDist) { minDist = d; target = e; }
        });
        if (target) {
            fireBullet(target);
...
function updateEnemies() {
    if (frameCount % GAME_CONFIG.enemySpawnRate === 0) {
        ...
        if (e) {
            ...
            e.active = true; e.x = player.x + Math.cos(a)*r; e.y = player.y + Math.sin(a)*r;
            e.hp = GAME_CONFIG.enemyMaxHp; e.maxHp = GAME_CONFIG.enemyMaxHp;
        }
    }
    enemies.forEach(e => {
        if (!e.active) return;
...
function showLevelUp() {
    ...
    ui.upgradeContainer.innerHTML = ''; // 기존 카드 제거
...
function draw() {
    ...
    enemies.forEach(e => { if(e.active) { 
        const sx=e.x-camera.x, sy=e.y-camera.y; if(onScreen(sx,sy)){ 
            ctx.fillStyle='red'; ctx.beginPath(); ctx.arc(sx,sy,GAME_CONFIG.enemyRadius,0,Math.PI*2); ctx.fill(); 
        } 
    }});
