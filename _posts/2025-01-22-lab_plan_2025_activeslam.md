...
> Node: 로봇의 시간별 자세(pose) 변경 가능
> Edge: pose간의 제약조건 $\rightarrow$ 변경 불가능 $\rightarrow$ 사실 변경 가능

최소 제곱 최적화를 통해서 제약 조건을 만족하는 노드 구성
Front-End: Node&Edge 그래프구성 + Loop Closing 감지
Back-End: 그래프 최적화를 통해 Node&Edge 재구성
결론: Loop Closing이 있으면 SLAM이 더 정확해진다
